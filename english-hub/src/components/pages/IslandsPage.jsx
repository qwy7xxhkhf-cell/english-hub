      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索句子..."
            className="ml-auto border border-stone-200 rounded-full px-3 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500 w-36" />
        </div>
                ...
                {filtered.map((s,i)=>{
                  ...
                  return (
                    <tr key={s.id} className={`border-b border-stone-50 hover:bg-stone-50 ${i%2===1?'bg-stone-50/40':''}`}>
                      ...
                      <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 rounded-full font-semibold`}>{s.level}</span></td>
                      <td className="px-4 py-3"><a href={s.audio} target="_blank" rel="noreferrer" className="text-xs bg-emerald-800 text-white px-2 py-0.5 rounded-full">▶</a></td>
        ...
        {view==='board' && (
          <div className="flex gap-4 overflow-x-auto pb-4">
            {['A1','A2','B1','B2'].map(lv=>{
              ...
              const colors = {A1:'bg-green-50 border-green-200 text-green-700',A2:'bg-sky-50 border-sky-200 text-sky-700',B1:'bg-amber-50 border-amber-200 text-amber-700',B2:'bg-rose-50 border-rose-200 text-rose-700'}
              return (
                <div key={lv} className="flex-none w-72">
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-lg mb-3 font-semibold text-sm border ${colors[lv]}`}>
                    ...
                    {cards.map(s=>(
                      <div key={s.id} className="bg-white border border-stone-200 rounded-xl p-3">
                        <div className="flex items-center justify-between mb-2">
                          <a href={s.audio} target="_blank" rel="noreferrer" className="text-xs bg-emerald-800 text-white px-2 py-0.5 rounded-full">▶ Play</a>
