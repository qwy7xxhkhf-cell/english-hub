...
function useTodayStats() {
  const key = `eh_vtoday_${new Date().toISOString().slice(0,10)}`
...
// ── Verb Study Mode ───────────────────────────────────────────
function VerbStudyMode({ verb, verbData, sentences, progress, toggleMastered, onClose }) {
  ...
  const key     = current ? `v_${verb.replace(/\s+/g,'_')}_${current.num}` : null
      ...
      <div className="flex-1 overflow-y-auto flex items-start justify-center px-4 py-6">
        <div className="w-full max-w-lg">
          ...
          <div className="flex items-center gap-2 mb-4">
            <span className="text-sm font-bold text-emerald-700">{verb}</span>
            <span className="text-sm text-stone-400">{verbData?.chinese}</span>
...
export default function PhrasalPage({ progress, toggleMastered }) {
  ...
  function selectVerb(verb) { setActive(verb); setShowDetail(true) }
          ...
          <button key={v.verb} onClick={() => selectVerb(v.verb)}
            className={`w-full text-left px-3 py-3 rounded-xl transition-all flex items-center justify-between
              ${active===v.verb&&showDetail ? 'bg-emerald-800 text-white'
                : v.verb===todayVerb&&!(active===v.verb&&showDetail) ? 'ring-1 ring-amber-300 bg-amber-50'
                : 'hover:bg-stone-200 text-stone-700'}`}>
            <div>
              <div className={`font-semibold text-sm ${active===v.verb&&showDetail?'text-white':'text-stone-800'}`}>{v.verb}</div>
              <div className={`text-xs mt-0.5 ${active===v.verb&&showDetail?'text-emerald-200':'text-stone-400'}`}>{v.chinese}</div>
            </div>
