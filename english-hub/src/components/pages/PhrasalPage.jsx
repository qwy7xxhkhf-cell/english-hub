...
export default function PhrasalPage({ progress, toggleMastered }) {
  ...
  const [showDetail, setShowDetail] = useState(false) // mobile: list vs detail
  ...
  const ListPanel = (
    <div className={`
      flex flex-col
      md:w-56 md:flex-shrink-0 md:border-r md:border-stone-200 md:bg-stone-50
    ...
    `}>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        {!search && (
          <div className="px-2 py-1 text-xs font-semibold text-amber-600 uppercase tracking-wider">
            Today ✨
          </div>
        )}
        {filtered.map(v => (
  ...
  const DetailPanel = (
    <div className={`
      ...
      ${showDetail ? 'flex flex-col w-full' : 'hidden md:flex md:flex-col'}
    `}>
      {/* Mobile back button */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <a href={s.audio} target="_blank" rel="noreferrer"
                          className="flex items-center gap-1 bg-emerald-800 hover:bg-emerald-700 text-white text-xs px-3 py-1 rounded-full transition-colors">
                          ▶ Play
                        </a>
